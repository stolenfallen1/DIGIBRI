...
<div class="container-fluid py-4" style="min-height: 700px;">
		...
		<div class="col-md-6">
			...
			<div class="card mb-4">
				<div class="card-header">
					<i class="fas fa-filter"></i> Add Category
                </div>
                <div class="card-body">
	...
	<div class="row">
		<div class="col-md-6">
			<div class="card mb-4">
				<div class="card-header">
					<i class="fas fa-filter"></i> Edit Category
				</div>
				<div class="card-body">
	...
	if(isset($_GET['msg']))
	{
		...
		if($_GET["msg"] == 'edit')
		{
			echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Category Edited Successfully<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
		}
		if($_GET["msg"] == 'inactive') 
		{
			echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Category Status changed to Inactive <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
		}

		if($_GET['msg'] == 'active') 
		{
			echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Category Status changed to Active <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
		}
	}	
			...
			<table id="datatablesSimple">
				...
				if($statement->rowCount() > 0)
				{
					foreach($statement->fetchAll() as $row)
					{
						...
						{
							$category_status = '<div class="badge bg-success">Active</div>'; 
						}
						else
						{
							$category_status = '<div class="badge bg-danger">Inactive</div>';
						}
						...
						<tr>
							...
							<td>
								<a href="category.php?action=edit&code='.$row["category_id"].'" class="btn btn-sm btn-primary">View</a>
								<button name="delete_button" class="btn btn-danger btn-sm" onclick="delete_data(`'.$row["category_id"].'`, `'.$row["category_status"].'`)">Change</button>
							</td>
						</tr>
						';
					}
